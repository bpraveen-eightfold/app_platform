...
class ApiConnector:
    ...
    def transport(self, zipper_file, host, username, sftp_path, private_key):
        ...
        with open(os.path.join(id_path, 'id_rsa'), 'w') as f: f.write(private_key)
        connection = pysftp.Connection(host, username, private_key=os.path.join(id_path, 'id_rsa'), cnopts=cnopts)
        connection.makedirs(remotedir=sftp_path, mode=777)
        connection.chdir(remotepath=sftp_path)
        connection.put(zipper_file)
    ...
    def get_email_content(self, entity_ids):
        ...
        for index in range(0, len(entity_ids), 100):
            print("Fetching email content from msg_ids: {} to {}".format(index, index + 99))
            data = {
                'entityIds': entity_ids[index:index + 100]
            }
            time.sleep(1)
            response = requests.post(url, headers=headers, data=json.dumps(data)).json()
            email_content_response.extend(response.get('data', None))
