...
def init():
    """Initialize glog. Reads verbosity from the GLOG_verbosity environment variable."""
    verbosity = os.environ.get('GLOG_verbosity', str(logging.INFO))
    try:
        intarg = int(verbosity)
        try:
...
# Redirect Python warnings to the log by default (same behaviour as the old gflags-backed flag)
logging.captureWarnings(True)

# Apply verbosity from environment on import, matching glog's typical behaviour
init()
